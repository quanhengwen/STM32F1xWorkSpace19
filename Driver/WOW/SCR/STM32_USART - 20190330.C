...
unsigned char* Get_Rxd_Addr(USART_TypeDef* USARTx);
unsigned char* Get_Txd_Addr(USART_TypeDef* USARTx);
unsigned short Set_Dma_InfoSize(USART_TypeDef* USARTx,unsigned short size);
unsigned short Get_Dma_InfoSize(USART_TypeDef* USARTx);
DMA_Channel_TypeDef* Get_Usart_Tx_DMA_Channel(USART_TypeDef* USARTx);
DMA_Channel_TypeDef* Get_Usart_Rx_DMA_Channel(USART_TypeDef* USARTx);
void ClearFlag_Usart_Tx_DMA_Channel(USART_TypeDef* USARTx);
void ClearFlag_Usart_Rx_DMA_Channel(USART_TypeDef* USARTx);
unsigned short Usart_Dma_Send(USART_TypeDef* USARTx,u8 *tx_buffer,u16 BufferSize);		//串口DMA发送程序，如果数据已经传入到DMA，返回Buffer大小，否则返回0
void	Usart_Dma_Initialize	(USART_TypeDef* USARTx,u16 BufferSize);	//串口DMA配置---不开中断，查询方式
...
*返回值			:	无
*******************************************************************************/
void	Usart_Dma_Initialize(USART_TypeDef* USARTx,u16 BufferSize)	//USART_DMA配置--查询方式，不开中断
{
	//1)**********定义变量	
...
*******************************************************************************/
unsigned short Usart_Dma_Send(USART_TypeDef* USARTx,u8 *tx_buffer,u16 BufferSize)		//串口DMA发送程序
{
...
u16	API_USART_ReadBufferIDLE(USART_TypeDef* USARTx,u8 *RevBuffer)	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数
{
	...
	DMA_Channel_TypeDef* DMAx_Channelrx;			//DMA接收通道请求信号---DMA串口接收由串口发起中断，因此此处接收通道中断不使用
	...
	{	
		uart_rxd	=	Get_Rxd_Addr(USARTx);
		dma_size	=	Get_Dma_InfoSize(USARTx);
		DMAx_Channelrx=Get_Usart_Rx_DMA_Channel(USARTx);
