...
//*****************CAN初始化
static void CAN_Configuration_NR(u32 CAN_BaudRate);				//CAN1配置---标志位查询方式，不开中断
static void CAN_Configuration_IT(u32 CAN_BaudRate);				//CAN1配置---中断方式
//*****************CAN滤波器配置
static void CAN_FilterInitConfiguration_StdData(u8 CAN_GROUP,u16 CAN_ID,u16 MaskId);	//CAN滤波器配置---标准数据帧模式
static void CAN_FilterInitConfiguration_ExtData(u8 CAN_GROUP,u32 CAN_ID,u32 MaskId);	//CAN滤波器配置---扩展数据帧模式
static unsigned char CAN_PinSet(CANRemap_TypeDef CANRemap_Status);		//CAN引脚配置函数


/*******************************************************************************
*函数名			:	api_can_configuration_std_NR
*功能描述		:	can标准数据帧模式配置--使用查询方式获取数据
*输入				: CAN_BaudRate---CAN波特率
							CAN_Group------CAN过滤器分组，可配置14个(0~13)
							CAN_ID---------CAN过滤ID
							MaskId---------过滤标识，按位匹配，例如CAN_ID为0x0001,如果MaskId也为0x0001则表示只此分组只接收0x0001ID的消息
														若MaskId为0xFFFF,表示接收与CAN_ID所有的位都相当的消息，
														若MaskId为0x0000,表示接收所有的消息			: 
*返回值			:	无
*修改时间		:	无
*修改说明		:	无
*注释				:	wegam@sina.com
...
void api_can_configuration_std_NR(u32 CAN_BaudRate,u8 CAN_Group,u16 CAN_ID,u16 MaskId)
{
	...
	CAN_FilterInitConfiguration_StdData(CAN_Group,CAN_ID,MaskId);	//CAN滤波器配置---标准数据帧模式
}
/*******************************************************************************
*函数名			:	api_can_configuration_ext_NR
*功能描述		:	can扩展数据帧模式配置--使用查询方式获取数据
*输入				: CAN_BaudRate---CAN波特率
							CAN_Group------CAN过滤器分组，可配置14个(0~13)
							CAN_ID---------CAN过滤ID
							MaskId---------过滤标识，按位匹配，例如CAN_ID为0x0001,如果MaskId也为0x0001则表示只此分组只接收0x0001ID的消息
...
{
	CAN_Configuration_NR(CAN_BaudRate);				//CAN1配置---标志位查询方式，不开中断
	CAN_FilterInitConfiguration_ExtData(CAN_Group,CAN_ID,MaskId);	//CAN滤波器配置---标准数据帧模式
}
//------------------------------------------------------------------------------
...
/*******************************************************************************
*函数名			:	api_can_configuration_std_IT
*功能描述		:	can标准数据帧模式配置--使用中断方式获取数据
*输入				: CAN_BaudRate---CAN波特率
							CAN_Group------CAN过滤器分组，可配置14个(0~13)
							CAN_ID---------CAN过滤ID
							MaskId---------过滤标识，按位匹配，例如CAN_ID为0x0001,如果MaskId也为0x0001则表示只此分组只接收0x0001ID的消息
...
{
	CAN_Configuration_IT(CAN_BaudRate);				//CAN1配置---标志位查询方式，不开中断
	CAN_FilterInitConfiguration_StdData(CAN_Group,CAN_ID,MaskId);	//CAN滤波器配置---标准数据帧模式
}
/*******************************************************************************
*函数名			:	api_can_configuration_ext_IT
*功能描述		:	can扩展数据帧模式配置--使用中断方式获取数据
*输入				: CAN_BaudRate---CAN波特率
							CAN_Group------CAN过滤器分组，可配置14个(0~13)
							CAN_ID---------CAN过滤ID
							MaskId---------过滤标识，按位匹配，例如CAN_ID为0x0001,如果MaskId也为0x0001则表示只此分组只接收0x0001ID的消息
