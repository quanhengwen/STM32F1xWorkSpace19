  ...
  unsigned short	BufferSize	=	0;			//剩余缓存大小
	...
	switch(*(u32*)&USARTx)
	{
		case USART1_BASE:
				...
				if(BufferSize<dma_size)	          //可用缓存小于预设大小，表示已使用
				{
		...
		case USART2_BASE:
				...
				if(BufferSize<dma_size)	    //缓存在减小，表示在使用
				{
...
static unsigned char Get_Txd_Status(USART_TypeDef* USARTx)		//获取串口发送状态
{
	...
	switch(*(u32*)&USARTx)
	{
		...
		case UART5_BASE:
				//接收状态检查
				bitstatus	=	USART_GetFlagStatus(UART5,USART_FLAG_TC);		//检测发送数据寄存器是否为空	RESET-非空，SET-空
				break;
		default:break;
	}
	return bitstatus;
...
)	//USART_DMA配置--查询方式，不开中断
{
	...
	unsigned short dma_size=0;
	
	u8*	uart_rxd	=	Get_Rxd_Addr(USARTx);
	
	if(0==uart_rxd)
		return;
	dma_size	=	Set_Dma_InfoSize(USARTx,BufferSize);
	//2)******************************DMA
	...
	switch(*(u32*)&USARTx)
	{
		case 	USART1_BASE:
					if(USART_GetITStatus(USART1,USART_IT_IDLE)||USART_GetFlagStatus(USART1,USART_FLAG_IDLE))
					{
						...
						//------重新指向接收缓冲区地址并使能DMA接收			
						DMA1_Channel5->CMAR=(u32)uart_rxd;							//重新设置DMA接收地址
						DMA1_Channel5->CNDTR=dma_size;			  //重新设置接收数据个数			
			...
			case 	USART2_BASE:
					if(USART_GetITStatus(USART2,USART_IT_IDLE)||USART_GetFlagStatus(USART2,USART_FLAG_IDLE))
					{
						...
						//------重新指向接收缓冲区地址并使能DMA接收			
						DMA1_Channel6->CMAR=(u32)uart_rxd;							//重新设置DMA接收地址
						DMA1_Channel6->CNDTR=dma_size;			  //重新设置接收数据个数
			...
			case 	USART3_BASE:
					if(USART_GetITStatus(USART3,USART_IT_IDLE)||USART_GetFlagStatus(USART3,USART_FLAG_IDLE))
					{
						...
						//------重新指向接收缓冲区地址并使能DMA接收			
						DMA1_Channel3->CMAR=(u32)uart_rxd;							//重新设置DMA接收地址
						DMA1_Channel3->CNDTR=dma_size;			  //重新设置接收数据个数
			...
			case 	UART4_BASE:
					if(USART_GetITStatus(UART4,USART_IT_IDLE)||USART_GetFlagStatus(UART4,USART_FLAG_IDLE))
					{
						...
						//------重新指向接收缓冲区地址并使能DMA接收			
						DMA2_Channel3->CMAR=(u32)uart_rxd;							//重新设置DMA接收地址
						DMA2_Channel3->CNDTR=dma_size;		    	//重新设置接收数据个数
...
*******************************************************************************/
unsigned short API_USART_DMA_Send(
									USART_TypeDef* USARTx,									//串口号--USART1,USART2,USART3,UART4;//UART5不支持DMA
									u8 *tx_buffer,													//待发送数据缓冲区地址
									u16 BufferSize													//设定发送数据大小
)		//串口DMA发送程序
{
	unsigned char*	uart_txd	=	Get_Txd_Addr(USARTx);
	unsigned char  	tc = Get_Txd_Status(USARTx);		//获取串口发送状态
	...
	switch(*(u32*)&USARTx)
	{
		case USART1_BASE:
            ...
            if(1  ==  tc)    //发送完成
            {
							memcpy(uart_txd,tx_buffer,BufferSize);
              ...
              DMA1_Channel4->CMAR 	=   (u32)uart_txd;			  //发送缓冲区
            ...
            if(1  ==  tc)    //发送完成
            {
              ...
              DMA1_Channel7->CMAR 	=(u32)uart_txd;				  //发送缓冲区
            ...
            if(1  ==  tc)    //发送完成
            {
              ...
              DMA1_Channel2->CMAR 	=(u32)uart_txd;				//发送缓冲区
            ...
            if(1  ==  tc)    //发送完成
            {
              ...
              DMA2_Channel5->CMAR 	=(u32)uart_txd;				//发送缓冲区
