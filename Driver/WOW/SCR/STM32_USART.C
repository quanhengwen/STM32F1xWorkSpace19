...
//--------USART全局变量定义
#define	uart_dma_buffer_size	280	//默认串口DMA接收缓冲大小,如果配置时未输入缓存大小时使用的默认值
...
unsigned short api_usart_dma_receive(USART_TypeDef* USARTx,u8 *RevBuffer)
{
	if(get_usart_rx_idle(USARTx))
		return get_usart_rx_dma_buffer(USARTx,RevBuffer);
	else
		return 0;
}
/*******************************************************************************
...
unsigned short api_rs485_dam_receive(RS485Def *pRS485,u8 *RevBuffer)	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数，然后重新将接收缓冲区地址指向RxdBuffer，
{
	...
	if(get_usart_tx_idle(USARTx))    //发送完成
	{
...
static void	usart_dma_initialize(USART_TypeDef* USARTx,u16 BufferSize)	//USART_DMA配置--查询方式，不开中断
{
	...
	//9.3)-------------------------------------清除DMA标志                                 					
	set_tx_dma_flag_clear(USARTx);	// 清除串口DMA方式发送中断全局标志
	set_rx_dma_flag_clear(USARTx);	// 清除串口DMA方式接收中断全局标志	
}
/*******************************************************************************
*函数名			:	USART_DMA_ConfigurationNr
*功能描述		:	USART_DMA配置--查询方式，不开中断
...
*注释				:	wegam@sina.com
*******************************************************************************/
static void set_tx_dma_flag_clear(USART_TypeDef* USARTx)
{
	if(USART1	==	USARTx)
	{
...
*注释				:	wegam@sina.com
*******************************************************************************/
static void set_rx_dma_flag_clear(USART_TypeDef* USARTx)
{
	if(USART1	==	USARTx)
	{
...
{	
	...
	if(USART_GetFlagStatus(USARTx,USART_FLAG_IDLE)||(0==capacity))		//检测发送数据寄存器是否为空	RESET-非空，SET-空，
	{
		rxdelay	=	0;
		set_rx_retry_count(USARTx,rxdelay);
		set_usart_rx_capacity_backup(USARTx,dma_conf_size);
		return 1;			//空闲
	}
