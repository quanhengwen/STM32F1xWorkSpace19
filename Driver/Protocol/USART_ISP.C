...
{
	ISP_Conf->ISP_DATA.ReceivedLen=API_USART_ReadBufferIDLE(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_RvBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数，然后重新将接收缓冲区地址指向RxdBuffer
...
{
	ISP_Conf->ISP_DATA.ReceivedLen=API_USART_ReadBufferIDLE(ISP_Conf->USARTx,(u8*)ISP_Conf->ISP_DATA.ISP_RvBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的
	if(ISP_Conf->ISP_DATA.ReceivedLen)
	{
...
void USM_BSP_GetAck(BSP_Conf_TypeDef *BSP_Conf)		//获取从机应答
{
	...
	RxNum=API_USART_ReadBufferIDLE(BSP_Conf->BSP_Port.USARTx,(u8*)BSP_Conf->BSP_DATA.BSP_RvBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的
