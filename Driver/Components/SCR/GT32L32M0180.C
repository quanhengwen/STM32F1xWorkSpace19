...
u32 gt32l32_get_address(u8 font, u8 c1, u8 c2, u8 c3, u8 c4);//获取地址
u32 gt32l32_get_length(u8 font, u8 c1, u8 c2, u8 c3, u8 c4);//获取长度
u16 gt32l32_read_buffer(u32 start_address,u32 lengh,u8 *ReadBuffer);		//从字库中读数据并返回数据长度

u32 gt32l32_get_address_GB18030(u8 c1, u8 c2, u8 c3, u8 c4);		//12x12点阵GB18030汉字&字符地址计算
...
{
  pSPI  = &pInfo->SPI;
  api_spi_configurationNR(pSPI);				//普通SPI通讯方式配置
...
u16 api_gt32l32_get_code(u8 font,u16 word,u8 *rxBuffer)
{
	...
	if(word>>8>=0x80)
	{
		Address	=	gt32l32_get_address(font, (u8)(word>>8), (u8)word, 0, 0);		//获取地址
		lengh= gt32l32_get_length(font, (u8)(word>>8), (u8)word, 0, 0);			//获取长度
	}
	else
	{
		Address	=	gt32l32_get_address(font, (u8)word, 0, 0, 0);			//获取地址
		lengh= gt32l32_get_length(font, (u8)word, 0, 0, 0);			//获取长度
	}	
  gt32l32_read_buffer(Address,lengh,rxBuffer);		//读取数据
	return lengh;
}
/*******************************************************************************
* 函数名			:	api_gt32l32_get_antenna
...
u32 api_gt32l32_get_antenna(u8	NUM,u8 *GetBuffer)
{
  ...
  gt32l32_read_buffer(Address,lengh,GetBuffer);		//从字库中读数据并返回数据长度
...
/*************************************************************************************************** 
32x32点阵GB18030汉字&字符
函数：u32 gt32l32_get_address_GB18030 (u8 c1, u8 c2, u8 c3, u8 c4)  
功能：计算汉字点阵在芯片中的地址 
...
//	SPI_Cmd(pInfo->SPI.port.SPIx, ENABLE);
	//____________发送地址数据	
	api_spi_ReadWrite_byte(&pInfo->SPI,Address);						//发送地址
	//____________接收数据	
	
	ChipStatus=api_spi_ReadWrite_byte(&pInfo->SPI,0XFF);// 从字库读出点阵数据到数组中。
...
void API_GT32L32M0180_SectorErase(unsigned short UserSectorNum)
{
	...
	spi_set_nss_low(pSPI);
	//____________写入命令
	api_spi_ReadWrite_byte(pSPI,CMD);						//发送地址
	//____________写入地址
	api_spi_ReadWrite_byte(pSPI,Address>>16);				//发送地址
	api_spi_ReadWrite_byte(pSPI,Address>>8);				//发送地址
	api_spi_ReadWrite_byte(pSPI,Address);						//发送地址
	//____________取消片选	
	spi_set_nss_high(pSPI);
}
/*******************************************************************************
...
void API_GT32L32M0180_PageWrite(unsigned short UserPageNum,unsigned char* UserData)
{
	...
	for(i=0;i<256;i++)
	{
		api_spi_ReadWrite_byte(pSPI,Buffer[i]);	//从字库读出点阵数据到数组中。
	}
	//____________取消片选	
	spi_set_nss_high(pSPI);
}
...
void GT32L32_ChipErase(GT32L32Def *pInfo)
{
	...
	for(i=0;i<len;i++)
	{
	 api_spi_ReadWrite_byte(&pInfo->SPI,0XFF);// 从字库读出点阵数据到数组中。
	}
	//____________取消片选	
	spi_set_nss_high(pSPI);
//	SPI_Cmd(pInfo->SPI.port.SPIx,DISABLE);
...
//	SPI_Cmd(pInfo->SPI.Port.SPIx, ENABLE);
	spi_set_nss_low(pSPI);
	start_address=start_address|0x03000000;		//0x03指令字+地址。
	//Address=Address|0x0B000000;//0x0B指令字+地址。--快速

	api_spi_ReadWrite_byte(pSPI,start_address>>24);				//发送高8位---命令
	api_spi_ReadWrite_byte(pSPI,start_address>>16);				//发送地址
	api_spi_ReadWrite_byte(pSPI,start_address>>8);				//发送地址
	api_spi_ReadWrite_byte(pSPI,start_address);						//发送地址
	
//	if((Address&0x0B000000)==0x0B000000)
//		api_spi_ReadWrite_byte(pInfo->SPI.Port.SPIx,0XFF);						//从字库读出点阵数据到数组中。
//	while((Status=GT32L32_ReadStatus()&0x01)!=0x01);
	for(i=0;i<lengh;i++)
	{
		ReadBuffer[i]=api_spi_ReadWrite_byte(pSPI,0XFF);	//从字库读出点阵数据到数组中。
	}
	//____________取消片选	
	spi_set_nss_high(pSPI);
//	SPI_Cmd(pInfo->SPI.Port.SPIx, DISABLE);
