...
#define	SetBQ26100slavePinOut		{	GPIOA->CRL&=0xF0FFFFFF;\
...
unsigned char api_bq26100slave_process(void)
{
	...
	if(0==GetBQ26100slavePinLevel)													//低电平
	{
		unsigned char bit=0;
		//========================位接收:启动或者干扰滤波处理
		bit=api_bq26100slave_receive_bit();
		if(0xFF==bit)
		{
		...
		//========================数据传输
		//------------------------bit有效信号:主机写0、1或者启动bit位读,读数据时bit只能为1
		else if(4>SDQ_SAMPLE->data.receivelen)
		{
			api_bq26100slave_Initialize(bit);
		}
		else
		{
			//api_bq26100slave_command_process(bit);
			if(0==api_bq26100slave_command_process(bit))
			{
				api_bq26100slave_default();
			}
		}
...
unsigned char api_bq26100slave_Initialize(unsigned char bit)
{
	...
	//---------------------等待接收完一字节
	if(1==api_bq26100slave_receive_byte(bit))	//未接收完一字节
	{
		//-------------------获取ROM命令
...
*注释				:	wegam@sina.com
*******************************************************************************/
unsigned char api_bq26100slave_read_digest(unsigned char bit)
{
	if(1	!=	bit)	//信号出错:主机启动读信号时传入的bit只能为1
	{
		return 0;
	}		
...
*注释				:	wegam@sina.com
*******************************************************************************/
unsigned char api_bq26100slave_write_message(unsigned char bit)
{
	//====================第一步:主机写数据
	if(sdq_master_write	==	SDQ_SAMPLE->status)
	{
		if(1==api_bq26100slave_receive_byte(bit))	//未接收完一字节
		{
			if(20>SDQ_SAMPLE->data.Message.MessageLen)	//message合法字节数据只能为20
			{
...
*注释				:	wegam@sina.com
*******************************************************************************/
unsigned char api_bq26100slave_read_control(unsigned char bit)
{
...
*注释				:	wegam@sina.com
*******************************************************************************/
unsigned char api_bq26100slave_write_control(unsigned char bit)
{
	//====================第一步:主机写数据
	if(sdq_master_write	==	SDQ_SAMPLE->status)
	{
...
//		while(0==GetBQ26100slavePinLevel)
//		{
//			//-------------连续低电平超过3us(1ms重装值为9000)
//			//-------------2us为18(8982-9000-2*9)
//			if(SysTick_GetVal()<8990)
//			{
//				return 1;
//			}
//		}
