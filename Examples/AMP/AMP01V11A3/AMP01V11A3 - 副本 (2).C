...
static char is_ack_frame(unsigned char* pBuffer,unsigned short len)
{
	ampphydef* frame	=	(ampphydef*)api_get_frame(pBuffer,len);
	if(frame)
	{
		unsigned char dir	=	frame->msg.cmd&0x80;
		ampcmddef			cmd	=	frame->msg.cmd&0x7F;
		if(ampCmdAck	==	cmd)
		{
			if(0x00==frame->msg.cmd)	//向下应答
				return	1;
			else if(0x01==frame->msg.cmd)	//向上应答
				return -1;
		}
		else
			return 0;
